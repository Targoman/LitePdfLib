...
FX_DWORD CPDF_Parser::StartParse(IFX_FileRead* pFileAccess, FX_BOOL bReParse, FX_BOOL bOwnFileRead)
{
    ...
    m_pDocument->LoadDoc();
    if (m_pDocument->GetRoot() == nullptr || m_pDocument->GetPageCount() == 0) {
        if (bXRefRebuilt) {
            return PDFPARSE_ERROR_FORMAT;
        }
        ReleaseEncryptHandler();
        ...
        m_pDocument->LoadDoc();
        if (m_pDocument->GetRoot() == nullptr) {
            return PDFPARSE_ERROR_FORMAT;
        }
    }
...
void CPDF_Parser::ReleaseEncryptHandler()
{
    if (m_Syntax.m_pCryptoHandler) {
        ...
        m_Syntax.m_pCryptoHandler = nullptr;
    }
                    ...
                    switch (inside_index) {
                        ...
                        case 3:
                            if (_PDF_CharType[byte] == 'W' || _PDF_CharType[byte] == 'D') {
                                ...
                                CPDF_Object *pObject = ParseIndirectObjectAtByStrict(m_pDocument, obj_pos, objnum, nullptr, &obj_end);
                                if (pObject) {
                        ...
                        if (_PDF_CharType[byte] == 'W' || _PDF_CharType[byte] == 'D') {
                            ...
                            if (pObj) {
                                if (pObj->GetType() != PDFOBJ_DICTIONARY && pObj->GetType() != PDFOBJ_STREAM) {
                                    ...
                                    CPDF_Dictionary* pTrailer = nullptr;
                                    ...
                                    if (pTrailer) {
                                        if (m_pTrailer) {
                                            ...
                                            if (pRoot == nullptr || (pRoot->GetType() == PDFOBJ_REFERENCE &&
...
{
    CPDF_Stream* pStream = (CPDF_Stream*)ParseIndirectObjectAt(m_pDocument, pos, 0, nullptr);
    if (!pStream) {
        return FALSE;
    }
    if (m_pDocument) {
...
{
    CPDF_Object* pID = m_pTrailer ? m_pTrailer->GetElement(FX_BSTRC("ID")) : nullptr;
    if (pID == nullptr) {
        return nullptr;
    }
    if (pID->GetType() == PDFOBJ_REFERENCE) {
        pID = ParseIndirectObject(nullptr, ((CPDF_Reference*)pID)->GetRefObjNum());
        m_pTrailer->SetAt(FX_BSTRC("ID"), pID);
    }
    if (pID == nullptr || pID->GetType() != PDFOBJ_ARRAY) {
        return nullptr;
    }
...
{
    CPDF_Reference* pRef = m_pTrailer ? (CPDF_Reference*)m_pTrailer->GetElement(FX_BSTRC("Root")) : nullptr;
    if (pRef == nullptr || pRef->GetType() != PDFOBJ_REFERENCE) {
        return 0;
    }
...
{
    CPDF_Reference* pRef = m_pTrailer ? (CPDF_Reference*)m_pTrailer->GetElement(FX_BSTRC("Info")) : nullptr;
    if (pRef == nullptr || pRef->GetType() != PDFOBJ_REFERENCE) {
        return 0;
    }
    return pRef->GetRefObjNum();
...
CPDF_StreamAcc* CPDF_Parser::GetObjectStream(FX_DWORD objnum)
{
    ...
    const CPDF_Stream* pStream = m_pDocument ? (CPDF_Stream*)m_pDocument->GetIndirectObject(objnum) : nullptr;
    if (pStream == nullptr || pStream->GetType() != PDFOBJ_STREAM) {
        return nullptr;
    }
...
CPDF_Dictionary* CPDF_Parser::LoadTrailerV4()
{
    ...
    CPDF_Object* pObj = m_Syntax.GetObject(m_pDocument, 0, 0, 0);
    if (pObj == nullptr || pObj->GetType() != PDFOBJ_DICTIONARY) {
        if (pObj) {
            pObj->Release();
        }
        return nullptr;
...
{
    return m_pSecurityHandler == nullptr ? TRUE : m_pSecurityHandler->IsOwner();
}

std::vector<FX_BYTE> CPDF_Parser::GetRawBuffer() const
{
    std::vector<FX_BYTE> Data;
    Data.assign(this->m_Syntax.m_pFileBuf,
                this->m_Syntax.m_pFileBuf + (size_t)((FX_FILESIZE)this->m_Syntax.m_BufSize > this->m_Syntax.m_FileLen ?
                                                         this->m_Syntax.m_FileLen : this->m_Syntax.m_BufSize));
    return Data;
}

void CPDF_Parser::SetSecurityHandler(CPDF_SecurityHandler* pSecurityHandler, FX_BOOL bForced)
{
    ASSERT(m_pSecurityHandler == nullptr);
    if (m_pSecurityHandler && !m_bForceUseSecurityHandler) {
        delete m_pSecurityHandler;
        m_pSecurityHandler = nullptr;
    }
    ...
    m_Syntax.m_pCryptoHandler->Init(nullptr, pSecurityHandler);
}
FX_BOOL CPDF_Parser::IsLinearizedFile(IFX_FileRead* pFileAccess, FX_DWORD offset)
{
    ...
    m_pLinearized = m_Syntax.GetObject(nullptr, objnum, gennum, 0);
    if (!m_pLinearized) {
        return FALSE;
    }
...
CPDF_Object* CPDF_SyntaxParser::GetObject(CPDF_IndirectObjects* pObjList, FX_DWORD objnum, FX_DWORD gennum, FX_INT32 level, PARSE_CONTEXT* pContext, FX_BOOL bDecrypt)
{
    ...
    if (word == FX_BSTRC("<<")) {
        ...
        if (IsSignatureDict(pDict)) {
            ...
            CPDF_Object* pObj = GetObject(pObjList, objnum, gennum, level + 1, nullptr, FALSE);
        ...
        FX_DWORD objnum, FX_DWORD gennum)
{
    ...
    if (pLenObj && ((pLenObj->GetType() != PDFOBJ_REFERENCE) ||
                    ((((CPDF_Reference*)pLenObj)->GetObjList() != nullptr) &&
...
std::vector<FX_BYTE> CPDF_SyntaxParser::GetBuffer()
{
    std::vector<FX_BYTE> Data;
    Data.assign(this->m_pFileBuf, this->m_pFileBuf + (size_t)((FX_FILESIZE)m_BufSize > m_FileLen ? m_FileLen : m_BufSize));
}
        ...
        switch (type) {
            ...
            case PDFOBJ_REFERENCE: {
                    ...
                    if (!m_pFileAvail->IsDataAvail(offset, size.ValueOrDie())) {
                        ...
                        CPDF_Object *pReferred = m_pDocument->GetIndirectObject(pRef->GetRefObjNum(), nullptr);
                        if (pReferred) {
...
CPDF_Object* CPDF_DataAvail::GetObject(FX_DWORD objnum, IFX_DownloadHints* pHints, FX_BOOL *pExistInFile)
{
    ...
    if (m_pDocument == nullptr) {
        ...
        pRet = m_parser.ParseIndirectObject(nullptr, objnum);
    ...
    CPDF_Parser *pParser = (CPDF_Parser *)(m_pDocument->GetParser());
    pRet = pParser->ParseIndirectObject(nullptr, objnum, nullptr);
    if (!pRet && pExistInFile) {
        *pExistInFile = FALSE;
    }
    return pRet;
...
FX_BOOL CPDF_DataAvail::PreparePageItem()
{
    ...
    CPDF_Reference* pRef = pRoot ? (CPDF_Reference*)pRoot->GetElement(FX_BSTRC("Pages")) : nullptr;
    if (pRef == nullptr || pRef->GetType() != PDFOBJ_REFERENCE) {
        m_docStatus = PDF_DATAAVAIL_ERROR;
        return FALSE;
    }
...
{
    if (nullptr == m_pageMapCheckState) {
...
FX_BOOL CPDF_DataAvail::GetPageKids(CPDF_Parser *pParser, CPDF_Object *pPages)
{
    ...
    CPDF_Object *pKids = pDict ? pDict->GetElement(FX_BSTRC("Kids")) : nullptr;
    if (!pKids) {
        return TRUE;
    }
...
FX_BOOL CPDF_DataAvail::CheckFirstPage(IFX_DownloadHints *pHints)
{
    ...
    CPDF_Object *pEndOffSet = pDict ? pDict->GetElement(FX_BSTRC("E")) : nullptr;
    if (!pEndOffSet) {
        m_docStatus = PDF_DATAAVAIL_ERROR;
        return FALSE;
    }
    CPDF_Object *pXRefOffset = pDict ? pDict->GetElement(FX_BSTRC("T")) : nullptr;
    if (!pXRefOffset) {
        m_docStatus = PDF_DATAAVAIL_ERROR;
        return FALSE;
    }
    CPDF_Object *pFileLen = pDict ? pDict->GetElement(FX_BSTRC("L")) : nullptr;
    if (!pFileLen) {
        m_docStatus = PDF_DATAAVAIL_ERROR;
        return FALSE;
    }
...
CPDF_Object	* CPDF_DataAvail::ParseIndirectObjectAt(FX_FILESIZE pos, FX_DWORD objnum)
{
    ...
    CPDF_Object* pObj = m_syntaxParser.GetObject(nullptr, objnum, gennum, 0);
    m_syntaxParser.RestorePos(SavedPos);
    return pObj;
}
...
FX_DWORD CPDF_DataAvail::CheckCrossRefStream(IFX_DownloadHints* pHints, FX_FILESIZE &xref_offset)
{
    ...
    if (m_pFileAvail->IsDataAvail(m_Pos, req_size)) {
        ...
        CPDF_Object *pObj = m_parser.ParseIndirectObjectAt(nullptr, 0, objNum, nullptr);
        if (!pObj) {
            m_Pos += m_parser.m_Syntax.SavePos();
            return 0;
        }
        CPDF_Dictionary* pDict = pObj->GetDict();
        CPDF_Object *pName = pDict ? pDict->GetElement(FX_BSTRC("Type")) : nullptr;
...
FX_BOOL CPDF_DataAvail::CheckTrailer(IFX_DownloadHints* pHints)
{
    ...
    if (m_pFileAvail->IsDataAvail(m_Pos, iTrailerSize)) {
        ...
        CFX_SmartPointer<IFX_FileStream> file(FX_CreateMemoryStream(pBuf, (size_t)iSize, FALSE));
        m_syntaxParser.InitParser((IFX_FileStream*)file, 0);
        CPDF_Object *pTrailer = m_syntaxParser.GetObject(nullptr, 0, 0, 0);
        if (!pTrailer) {

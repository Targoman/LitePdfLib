...
static void _UpdateLineEndPoints(CFX_FloatRect& rect, FX_FLOAT start_x, FX_FLOAT start_y, FX_FLOAT end_x, FX_FLOAT end_y,
                                 FX_FLOAT hw, LineCap line_cap)
{
    FX_FLOAT offset = line_cap != LineCap::LineCapButt ? hw : 0.f;
    if (start_x == end_x) {
        if (start_y == end_y) {
            rect.UpdateRect(end_x + offset, end_y + offset);
            rect.UpdateRect(end_x - offset, end_y - offset);
            return;
        }
        FX_FLOAT point_y;
        if (end_y < start_y) {
            point_y = end_y - offset;
        } else {
            point_y = end_y + offset;
        }
                                  ...
                                  FX_FLOAT half_width, FX_FLOAT miter_limit)
{
    ...
    if (FXSYS_fabs(start_k - end_k) < 1.0f / 20) {
        ...
        if (start_dir == end_dir) {
            _UpdateLineEndPoints(rect, middle_x, middle_y, end_x, end_y, half_width, LineCap::LineCapSquare);
        } else {
            _UpdateLineEndPoints(rect, start_x, start_y, middle_x, middle_y, half_width, LineCap::LineCapSquare);
        }
        return;
    }
...
CFX_FloatRect CFX_PathData::GetBoundingBox(FX_FLOAT line_width, LineCap line_cap, FX_FLOAT miter_limit) const
{
    CFX_FloatRect rect(100000 * 1.0f, 100000 * 1.0f, -100000 * 1.0f, -100000 * 1.0f);
    int iPoint = 0;
    FX_FLOAT half_width = line_width / 2.f;
    ...
    while (iPoint < m_PointCount) {
        ...
        if (bJoin) {
            ...
            _UpdateLineEndPoints(rect, start_x, start_y, end_x, end_y, half_width, line_cap);
        }
        iPoint ++;
    }
    return rect;
